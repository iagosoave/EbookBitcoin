...
// Updated FloatingNotification for better mobile visibility
const FloatingNotification = ({ notification, onComplete }) => (
  <motion.div
    ...
    className="fixed left-2 md:left-4 z-50 max-w-[calc(100vw-1rem)] md:max-w-sm"
    style={{ top: `${notification.position}px` }}
  >
    <PremiumCard className="p-3 md:p-4">
      <div className="flex items-center space-x-3">
        <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-yellow-400/20 flex items-center justify-center flex-shrink-0">
          {notification.icon}
        </div>
        <div className="flex-1">
          <p className="text-yellow-400 text-xs md:text-sm">{notification.text}</p>
...
// Updated SpotsWarning for mobile
const SpotsWarning = ({ spots }) => (
  <motion.div
    initial={{ y: 20, opacity: 0 }}
    animate={{ y: 0, opacity: 1 }}
    exit={{ y: 20, opacity: 0 }}
    className="fixed bottom-4 right-2 md:right-4 z-50 max-w-[calc(100vw-1rem)]"
  >
    <PremiumCard className="p-3 md:p-4 bg-red-500/10">
      <div className="flex items-center space-x-3">
...
// Updated Timer for mobile
const Timer = ({ minutes, seconds }) => (
  <div className="flex items-center space-x-2 md:space-x-4">
    <div className="w-10 h-10 md:w-14 md:h-14 rounded-xl bg-yellow-400/20 flex items-center justify-center">
      <Clock className="w-5 h-5 md:w-7 md:h-7 text-yellow-400" />
    </div>
    <div>
      <p className="text-gray-400 text-xs md:text-sm">Oferta expira em</p>
      <div className="flex items-baseline space-x-1">
        <span className="text-xl md:text-2xl font-bold text-yellow-400">
          {minutes}:{seconds.toString().padStart(2, '0')}
        </span>
        <span className="text-yellow-400/60 text-sm">min</span>
      </div>
    </div>
  </div>
);

// Updated LiveCounter for mobile
    ...
    <section className="min-h-screen relative overflow-hidden bg-black">
      ...
      <div className="container mx-auto px-4 relative z-10 py-12 md:py-24">
        <div className="max-w-6xl mx-auto">
          ...
          >
            <PremiumCard className="inline-block px-4 md:px-8 py-3 md:py-4">
              <div className="flex flex-col md:flex-row items-center md:justify-center space-y-4 md:space-y-0 md:space-x-6 md:divide-x md:divide-yellow-400/20">
                <Timer minutes={timer.minutes} seconds={timer.seconds} />
                
                <div className="flex items-center space-x-3 md:pl-6">
          ...
          <div className="grid lg:grid-cols-2 gap-8 md:gap-16 items-center">
            ...
            >
              <div className="space-y-6 md:space-y-8">
                ...
                >
                  <PremiumCard className="px-4 md:px-6 py-2">
              ...
              <div className="grid grid-cols-2 gap-4 md:gap-6">
                {[
                  ...
                  <PremiumCard key={index} className="p-3 md:p-4" hoverEffect>
                    <motion.div
                      className="flex items-center space-x-3 md:space-x-4"
                      whileHover={{ scale: 1.02 }}
                    >
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-yellow-400/20 flex items-center justify-center">
                        <stat.icon className="w-5 h-5 md:w-6 md:h-6 text-yellow-400" />
                      </div>
                      <div>
                        <span className="block text-xl md:text-2xl font-bold bg-gradient-to-r from-yellow-400 to-yellow-200 bg-clip-text text-transparent">
                          {stat.number}
                        </span>
            ...
            >
              <PremiumCard className="p-6 md:p-8" glowEffect>
                {/* Floating Bonus Banner */}
                <div className="absolute -top-4 md:-top-5 left-1/2 transform -translate-x-1/2 w-full max-w-[90%] md:max-w-sm">
                  <PremiumCard className="px-4 md:px-6 py-2">
                    <motion.div
                      animate={{ scale: [1, 1.05, 1] }}
                      transition={{ duration: 1.5, repeat: Infinity }}
                ...
                <div className="space-y-6 md:space-y-8 mt-6">
                  ...
                  <div className="text-center">
                    <h3 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-yellow-400 to-yellow-200 bg-clip-text text-transparent mb-2">
                      Método Completo + Bônus
                    </h3>
                    <p className="text-gray-400 text-sm md:text-base">Acesso vitalício garantido</p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 md:gap-4">
                    {[
                      ...
                      <motion.div
                        ...
                        className="group"
                      >
                        <div className="flex flex-col items-center text-center space-y-2 md:space-y-3 p-2 md:p-4">
                          <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-yellow-400/20 flex items-center justify-center group-hover:bg-yellow-400/30 transition-colors">
                            <feature.icon className="w-5 h-5 md:w-6 md:h-6 text-yellow-400" />
                          </div>
                          <span className="text-gray-300 font-medium text-sm md:text-base group-hover:text-yellow-400 transition-colors">
                            {feature.text}
                          </span>
                        </div>
                      </motion.div>
                    ...
                    {[
                      ...
                      >
                        <div className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-yellow-400/20 flex items-center justify-center">
                          <Check className="w-3 h-3 md:w-4 md:h-4 text-yellow-400" />
                        </div>
                        <span className="text-gray-300 text-sm md:text-base">{bonus}</span>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  >
                    ...
                    <div className="relative bg-gradient-to-r from-yellow-500 to-yellow-400 text-black font-bold py-4 md:py-6 px-6 md:px-8 rounded-xl text-lg md:text-xl shadow-lg shadow-yellow-400/20">
                  ...
                  {/* Trust Elements */}
                  <div className="grid grid-cols-2 gap-3 md:gap-4 text-xs md:text-sm text-gray-400">
